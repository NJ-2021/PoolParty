...
export const getTransactionOnSepoliaByHash = async (txHash: string): Promise<TransactionDetails> => {
  const response = await fetch(`https://eth-sepolia.blockscout.com/api/v2/transactions/${txHash}`);
  return response.json();
};

export const getTokenTransfersOnEthSepolia = async (address: string) => {
  const response = await fetch(`https://eth-sepolia.blockscout.com/api/v2/addresses/${address}/token-transfers?type=`);
  const json = await response.json();
  return json.items;
};

export const getTransactionsOnEthSepolia = async (address: string) => {
